...
export default function HeroCarousel({ banners }: { banners: Banner[] }) {
  ...
  const inner = (
    <div className="relative h-full w-full overflow-hidden rounded-2xl border border-pink-200 bg-white">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-8">
        ...
        {current.ctaLabel && current.linkUrl && (
          <span className="mt-4 inline-flex items-center gap-2 rounded-xl bg-pink-500 px-5 py-2 text-sm font-semibold text-black shadow-lg">
            {current.ctaLabel}
          </span>
        )}
      </div>
        ...
        {banners.length > 1 && (
          <>
            ...
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
              {banners.map((_, i) => (
                <button key={i} onClick={() => setIdx(i)} aria-label={`Slide ${i + 1}`} className={`h-1.5 rounded-full transition-all ${i === idx ? "w-6 bg-pink-500" : "w-1.5 bg-white/40"}`} />
              ))}
            </div>
          </>
        )}
